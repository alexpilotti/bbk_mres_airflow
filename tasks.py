...
TRAINING_INPUT_PATH = f"{common.DATA_PATH}/S_FULL.parquet"
...
UCL_TRAINING_INPUT_PATH = f"{UCL_DATA_PATH}/S_FULL.parquet"
...
def create_attention_comparison_tasks(
    ...
    if not pre_trained and not model_path:
...
def create_embeddings_tasks(ssh_hook, sftp_hook, model, chain,
    ...
    output_path = SVM_EMBEDDINGS_PREDICTION_OUTPUT_PATH.format(
        model=task_model_name, chain=chain, pre_trained=pre_trained_str)
